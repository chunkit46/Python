def findWinner(B: list[str]) -> str:
    # Check rows for sequences of 4 consecutive pieces
    for i in range(len(B)):
        for j in range(len(B[i]) - 3):
            if B[i][j] != '.' and B[i][j] == B[i][j + 1] and B[i][j + 1] == B[i][j + 2] and B[i][j + 2] == B[i][j + 3]:
                return B[i][j]

    # Check columns for sequences of 4 consecutive pieces
    for i in range(len(B) - 3):
        for j in range(len(B[i])):
            if B[i][j] != '.' and B[i][j] == B[i + 1][j] and B[i + 1][j] == B[i + 2][j] and B[i + 2][j] == B[i + 3][j]:
                return B[i][j]

    # Check diagonals for sequences of 4 consecutive pieces
    for i in range(len(B) - 3):
        for j in range(len(B[i]) - 3):
            if B[i][j] != '.' and B[i][j] == B[i + 1][j + 1] and B[i + 1][j + 1] == B[i + 2][j + 2] and B[i + 2][
                j + 2] == B[i + 3][j + 3]:
                return B[i][j]
            if B[i][j + 3] != '.' and B[i][j + 3] == B[i + 1][j + 2] and B[i + 1][j + 2] == B[i + 2][j + 1] and \
                    B[i + 2][j + 1] == B[i + 3][j]:
                return B[i][j + 3]
